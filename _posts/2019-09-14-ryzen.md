...
最近は Ricty の他にも色々フォントが出てきているようだが、Emacs の既存設定もあるのでとりあえず Ricty を使う。公式で配布しているスクリプトを使い生成するのが一般的だが、面倒臭いので手持ちの ttf ファイルをコピーした。

```console
$ mkdir ~/.fonts
$ cp Ricty-Regular.ttf Ricty-Bold.ttf ~/.fonts/
```
...
$ ~/.cargon/bin/cargo install racer
$ cd ~/Download/src
