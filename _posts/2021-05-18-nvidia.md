...
基本的には下記のページに従って実施しただけのため新しい情報は無いが、それだとあんまりなので少し勉強しながら解説を加えている。
...
Fedora といえばもう Wayland がディスプレイサーバーのデフォルトだが、うちでは X11 に戻している。サブディスプレイの明るさを調整できないからだ。X11 では `xrandr` を使えば調整できる。
...
Nouveau は NVIDIA 互換なオープンソースのドライバである [^11]。ドライバが複数居ると競合してしまうから、まずこれを無効化する必要がある。
...
いよいろ NVIDIA ドライバのインストールに入っていくが、ここまでで Nouveau を無効化してしまったので、再起動後に GUI を表示できない。そこで Runlevel を 3 すなわち GUI 無しのマルチユーザーモードに設定しておく。
...
マズいことになっていなければ、普通にログインシェルが立ち上がる。

ログインできたら、やっと初めにダウンロードしたインストーラの出番になる。
先に必要なパッケージを DNF でインストールしたあと、NVIDIA ドライバのインストーラを実行する。いくつか対話的に質問が表示されるが基本的に "YES" を選んでおけば問題無い。
...
しばしの処理の後、"Completed" 的なメッセージが表示されたら成功だ。やった!!

最後に Runlevel を 5 すなわち GUI 起動有りに戻して再起動する。
無事にデスクトップの画面が表示されれば完了である。
